...
int fire(struct Car *head) {
    int count = 0, sum_people = 0;
    for( struct Car * cur = head->next; cur != NULL; count++, cur = cur->next ){
        sum_people += cur->passenger;
        // printf("%d: p=%d f=%d\n", count, cur->passenger, cur->flammable);
        if(cur->passenger >= cur->flammable | sum_people >= cur->flammable){
            break;
        }
    }
    return count;

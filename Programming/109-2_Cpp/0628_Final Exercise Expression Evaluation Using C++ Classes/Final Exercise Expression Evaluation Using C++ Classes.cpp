...
//To be finished below...............   
    ...
    for(i=0; expr[i]; ++i){
        if (isdigit(expr[i]));
        push(intStack, expr[i]-"0"); //這個不知道為什麼？？？
        
        else{
            popVal1 = pop(intStack);
            popVal2 = pop(intStack);
            calculate(popVal1, popVal2, expr[i]);
        }
    }
...
{
   switch( oper ) {
       case "+": push(intStack, op1+op2); break;
       case "-": push(intStack, op1-op2); break;
       case "*": push(intStack, op1*op2); break;
       case "/": push(intStack, op2/op1); break;
